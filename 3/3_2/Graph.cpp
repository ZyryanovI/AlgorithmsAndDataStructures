...
Graph::Graph(unsigned int size) :
	size_(size),
	matrix_(size_, std::vector<double>(size_, 0.0)) {}
...
void Graph::getRelated(unsigned vertex, std::vector<pair_ud> &vertices) const
{
	vertices.clear();
	for (unsigned i = 0; i < size_; i++) {
		if (matrix_[vertex][i] != 0)
			vertices.push_back(std::make_pair(i, matrix_[vertex][i]));
	}
}
