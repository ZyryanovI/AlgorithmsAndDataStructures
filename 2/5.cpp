...
int Find_heurictics(std::vector<std::vector<int>> &table) //+
{
	...
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 4; ++j)
		{
			if (table[i][j] != 0)
			{
				heur += abs(i - (table[i][j] - 1) / 4) + abs(j - (table[i][j] - 1) % 4);
			}
		}
	}
	return heur;
...
void Solution(std::vector<std::vector<int>> &table)
{
	...
	else
	{
		...
		while (!queue_for_bfs.empty())
		{
			...
			if ((heuristic == 0) && (cur_pos.steps < 81))
			{
				std::cout << cur_pos.steps << std::endl;
			...
			if (cur_pos.steps + heuristic > 81)
			{
				continue;
			}

			if ((cur_pos.x != 0) && ((cur_pos.pref.size() == 0) || (cur_pos.pref[cur_pos.pref.size() - 1] != 'U'))) //чтобы не циклились
			{
			...
			if ((cur_pos.x != 3) && ((cur_pos.pref.size() == 0) || (cur_pos.pref[cur_pos.pref.size() - 1] != 'D')))
			{
				std::swap(cur_pos.table[cur_pos.x][cur_pos.y], cur_pos.table[cur_pos.x + 1][cur_pos.y]);
			...
			if ((cur_pos.y != 0) && ((cur_pos.pref.size() == 0) || (cur_pos.pref[cur_pos.pref.size() - 1] != 'L')))
			{
			...
			if ((cur_pos.y != 3) && ((cur_pos.pref.size() == 0) || (cur_pos.pref[cur_pos.pref.size() - 1] != 'R')))
			{
				std::swap(cur_pos.table[cur_pos.x][cur_pos.y], cur_pos.table[cur_pos.x][cur_pos.y + 1]);
